...
<div class="templatemo-table">
    <table class="MyTable">
        ...
        <tbody>
        <c:forEach items="${teamsList.teams}" var="team" >
            <tr>
                <td><a href="${pageContext.request.contextPath}/teamHasPlayers?idTeam=${team.id}">${team.name}</a></td>
                <td><p>${team.country}</p></td>
                <td>${team.city}</td>
                <td><a href="#" onclick="checkDeleteItem('${hostContext}/myTeamDelete?username=${loginDto.getUsername()}&idTeam=${team.id}')">Delete</a></td>
                </td>
            </tr>
        </c:forEach>
        </tbody>
